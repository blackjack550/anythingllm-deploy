...
│  └─────────────────────────────────────────────────────┘ │
│                                                          │
│  出网代理: 10.5.254.8:8118 (SearXNG 引擎访问外网)       │
└──────────────────────────────────────────────────────────┘
```
...
| **8118** | HTTP 代理 | 10.5.254.8 | SearXNG 出网代理（远程） |
...
**端口**: 8080  
**配置目录**: `/root/searxng/`  
**出网代理**: `http://10.5.254.8:8118` / `https://10.5.254.8:8118`  
**当前状态**: **88 个引擎可用**
...
outgoing:
  ...
  proxies:
    all://:
    - http://10.5.254.8:8118
    - https://10.5.254.8:8118
```
...
| **SearXNG 引擎批量失效** | `cd /root/searxng && python3 settings_check.py check`；检查代理 `10.5.254.8:8118` 是否存活 |
...
2. **SearXNG 代理是关键瓶颈**：所有引擎出网都走 `10.5.254.8:8118`，代理挂了所有引擎都会超时。`settings_check.py` 检测时强制 `proxies=None` 直连本地容器，避免代理问题干扰判断。
